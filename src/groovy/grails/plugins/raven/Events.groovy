...
class Events {
     ...
     * }
     */
    public static String build(String eventId, String message, String checksum, Throwable exception, String loggerClass, String logLevel, String ndc, Map<String, String> mdc, HttpServletRequest request, User user, String timestamp, Configuration config) {
        ...
        if (ndc || mdc) {
            ...
            obj.put("extra", embeddedJson)
            if (ndc) {
                embeddedJson.put(LOG4J_NDC, ndc)
            }
            if (mdc) {
                for (Map.Entry<String, Object> mdcEntry : mdc.entrySet()) {
                    embeddedJson.put(mdcEntry.getKey(), mdcEntry.getValue());
                }
            }
        }
        return obj.toString()
